...
addItem: (product: Product) => {
			const normalizedProduct = {
				...product,
				image: product.image || product.image_url ? `/images/${product.image || product.image_url}` : '/images/placeholder.png'
			};
			update((cart) => {
				...
				if (existing) {
					...
					cart.items.push({ ...normalizedProduct, quantity: 1 } as CartItem);
				}
